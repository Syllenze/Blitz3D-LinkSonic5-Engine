; Include file for FastImage library
...
;данные флаги - константы обязательно пригодятся
...
;используем глобальную переменную FI_Property для получения конкретного свойства из набора
...
;используем глобальную переменную FI_ImageProperty для получения конкретного свойства
...
;тип для получения всей информации о положении точки на примитиве (команда TestPoint)
Type FI_TestType
	Field Result%
...
;функция универсального "ручного" бленда (смешивания цветов)
;создана для начинающих, еще не освоивших DirectX7
;значения src и dest должны быть в пределах от 1 до 10
...
;вспомогательные функции, позволяющие не задавать каждый раз параметры с дефолтными значениями
