Type Template
	...
	Field size_add#, size_sub#, size_mult#                                     ;Size velocity
...
SetTemplateSizeVel(template, 0, 0, 1)
...
Function SetTemplateEmitterBlend(template, emitter_blend, emitter_fx=0)
tmp.Template = Object.Template(template)
tmp\emitter_blend = emitter_blend
tmp\emitter_fx = emitter_fx
End Function
...
Function SetTemplateVelocity(template, min_xv#, max_xv#, min_yv#, max_yv#, min_zv#, max_zv#, faceowner=False)
tmp.Template = Object.Template(template)
tmp\min_xv# = min_xv#
tmp\max_xv# = max_xv#
tmp\min_yv# = min_yv#
...
Function SetTemplateSizeVel(template, size_add#, size_sub#, size_mult#)
tmp.Template = Object.Template(template)
tmp\size_add# = size_add#
tmp\size_sub# = size_sub#
...
If e\tmp\emitter_fx <> 0 Then
EntityFX e\ent, e\tmp\emitter_fx
Else
EntityFX e\ent, 34
End If
		...
		If e\cnt_loop = 0 And e\del = False Then
			For i = 1 To e\tmp\particles_per_interval
				If (e\tmp\max_particles > -1 And cnt_particles < e\tmp\max_particles) Or e\tmp\max_particles = -1 Then
					...
					If e\tmp\faceowner = True Then
						p\xv# = Sin(EntityYaw#(e\owner, True)+Rnd(e\tmp\min_xv#, e\tmp\max_xv#))*Rnd(e\tmp\min_zv#, e\tmp\max_zv#)
						p\yv# = Rnd(e\tmp\min_yv#, e\tmp\max_yv#)
						p\zv# = -Cos(EntityYaw#(e\owner, True)+Rnd(e\tmp\min_xv#, e\tmp\max_xv#))*Rnd(e\tmp\min_zv#, e\tmp\max_zv#)					
					Else
...
cam_pitch# = EntityPitch(ParticleCam, True)
cam_yaw# = EntityYaw(ParticleCam, True)
cam_roll# = EntityRoll(ParticleCam, True)
For p.Particle = Each Particle
	...
	Else
		If p\emitter\frozen = False Then
			...
			p\sx# = p\sx# - p\emitter\tmp\size_sub#
			p\sy# = p\sy# - p\emitter\tmp\size_sub#			
		EndIf
