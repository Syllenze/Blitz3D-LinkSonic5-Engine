...
While Not KeyDown(1)
	...
	; Read input keys and change effect variables
		...
		If KeyHit(2) Then
			...
			DofFar = DofFar + 20
		EndIf
		If KeyHit(3)>0 And DofNear>10 Then
			DofNear = DofNear - 10
			DofFar = DofFar - 20
		EndIf
		If KeyHit(4)>0 And DofBlurPasses<16 Then DofBlurPasses = DofBlurPasses + 1
		If KeyHit(5)>0 And DofBlurPasses>1 Then DofBlurPasses = DofBlurPasses - 1
		If KeyHit(6)>0 And DofBlurRadius<10.0 Then DofBlurRadius = DofBlurRadius + 0.01
		If KeyHit(7)>0 And DofBlurRadius>0.01 Then DofBlurRadius = DofBlurRadius - 0.01
		If KeyHit(8) Then DofDirection = 1 - DofDirection
		If KeyHit(9) Then DofQuality = 1 - DofQuality
	...
	RenderPostprocess FE_DOF			; <<<< 	Рендерим пост-процесс эффекты после команды RenderWorld
...
Function DrawHelper()
	...
	Else
		...
		Text 10,70,"keys 1/2 - increase/decrease DOF range (near: "+Str(DofNear)+" far: "+Str(DofFar)+")"
		Text 10,90,"keys 3/4 - increase/decrease DOF blur passes count: "+Str(DofBlurPasses)
		Text 10,110,"keys 5/6 - increase/decrease DOF blur radius: "+Str(DofBlurRadius)
		Text 10,130,"key 7 - change DOF direction: "+Str(DofDirection)
		Text 10,150,"key 8 - change DOF quality: "+Str(DofQuality)
	EndIf
End Function