...
; Variables for adjusting the effect
	Global InverseAlpha# = 1.0
	Global InverseColorRed = 255
	Global InverseColorGreen = 255
	Global InverseColorBlue = 255
	Global InverseAlphaTexture = 0
...
While Not KeyDown(1)
	...
	; Read input keys and change effect variables
		...
		If KeyHit(2)>0 And InverseAlpha<1 Then InverseAlpha = InverseAlpha + 0.1
		If KeyHit(3)>0 And InverseAlpha>0 Then InverseAlpha = InverseAlpha - 0.1
		If KeyHit(4) Then
			InverseColorRed = Rand(0,255)
			InverseColorGreen = Rand(0,255)
			InverseColorBlue = Rand(0,255)
		EndIf
		If KeyHit(5) Then InverseAlphaTexture = AlphaTexture - InverseAlphaTexture
	
	
	
	; Customize effect
	CustomPostprocessInverse InverseAlpha, InverseColorRed, InverseColorGreen, InverseColorBlue, InverseAlphaTexture
	
	
	RenderPostprocess FE_Inverse		; <<<< 	Рендерим пост-процесс эффекты после команды RenderWorld
...
Function DrawHelper()
	...
	Else
		...
		Text 10,70,"keys 1/2 - increase/decrease Inverse alpha value: "+Str(InverseAlpha)
		Text 10,90,"key 3 - change Inverse color (random)"
		Text 10,110,"key 4 - enable/disable Inverse alpha-texture: $"+Hex(InverseAlphaTexture)
	EndIf
End Function