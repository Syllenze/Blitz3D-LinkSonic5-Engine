...
While Not KeyDown(1)
	...
	; Read input keys and change effect variables
		...
		If KeyHit(2)>0 Then BlurAngle = BlurAngle + 10
		If KeyHit(3)>0 Then BlurAngle = BlurAngle - 10
		...
		If KeyHit(8)>0 And BlurDistance<10.0 Then BlurDistance = BlurDistance + 0.01
		If KeyHit(9)>0 And BlurDistance>0.01 Then BlurDistance = BlurDistance - 0.01
		If KeyHit(10) Then BlurQuality = (1 + BlurQuality) And 3
		If KeyHit(11) Then
			BlurColorRed = Rand(0,255)
			BlurColorGreen = Rand(0,255)
			BlurColorBlue = Rand(0,255)
		EndIf
		If KeyHit(12) Then BlurAlphaTexture = AlphaTexture - BlurAlphaTexture
	
	
	
	; Customize effect
	CustomPostprocessBlurDirectional BlurAngle, BlurAlpha, BlurPasses, BlurDistance, BlurQuality, BlurColorRed, BlurColorGreen, BlurColorBlue, BlurAlphaTexture
	
	
	RenderPostprocess FE_BlurDirectional		; <<<< 	Рендерим пост-процесс эффекты после команды RenderWorld
...
Function DrawHelper()
	...
	Else
		...
		Text 10,70,"keys 1/2 - increase/decrease Blur angle: "+Str(BlurAngle)
		Text 10,90,"keys 3/4 - increase/decrease Blur alpha value: "+Str(BlurAlpha)
		Text 10,110,"key 5/6 - increase/decrease Blur passes: "+Str(BlurPasses)
		Text 10,130,"key 7/8 - increase/decrease Blur distance: "+Str(BlurDistance)
		Text 10,150,"key 9 - change Blur quality: "+Str(BlurQuality)
		Text 10,170,"key 0 - change Blur color (random)"
		Text 10,190,"key ''minus'' - enable/disable Blur alpha-texture: $"+Hex(BlurAlphaTexture)
	EndIf
End Function