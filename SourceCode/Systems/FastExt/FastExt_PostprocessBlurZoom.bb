...
; Variables for adjusting the effect
	...
	Global BlurZoomFactor# = 104
	Global BlurAlpha# = 1.0
	Global BlurPasses = 4
	Global BlurQuality = 1
		; quality 0-1 with small graphics bugs, but maximum fast
		; quality 2-3 without graphic bugs, but middle speed
...
While Not KeyDown(1)
	...
	; Read input keys and change effect variables
		...
		If KeyHit(2)>0 And BlurZoomFactor<200 Then BlurZoomFactor = BlurZoomFactor + 1
		If KeyHit(3)>0 And BlurZoomFactor>50 Then BlurZoomFactor = BlurZoomFactor - 1
		...
		If KeyHit(8)>0 And BlurQuality>0 Then BlurQuality = BlurQuality - 1
		If KeyHit(9)>0 And BlurQuality<3 Then BlurQuality = BlurQuality + 1
		If KeyHit(10) Then
			BlurColorRed = Rand(0,255)
			BlurColorGreen = Rand(0,255)
			BlurColorBlue = Rand(0,255)
		EndIf
		If KeyHit(11) Then BlurAlphaTexture = AlphaTexture - BlurAlphaTexture
	
	
	; Customize effect
	CustomPostprocessBlurZoom BlurCenterX, BlurCenterY, BlurZoomFactor, BlurAlpha, BlurPasses, BlurQuality, BlurColorRed, BlurColorGreen, BlurColorBlue, BlurAlphaTexture
	
	
	RenderPostprocess FE_BlurZoom		; <<<< 	Рендерим пост-процесс эффекты после команды RenderWorld
...
Function DrawHelper()
	...
	Else
		...
		Text 10,90,"keys 1/2 - increase/decrease Blur zoom factor: "+Str(BlurZoomFactor)
		...
		Text 10,150,"key 7/8 - change Blur quality: "+Str(BlurQuality)
		Text 10,170,"key 9 - change Blur color (random)"
		Text 10,190,"key 0 - enable/disable Blur alpha-texture: $"+Hex(BlurAlphaTexture)
	EndIf
End Function