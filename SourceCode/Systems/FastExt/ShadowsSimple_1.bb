	...
	If ShadowInitFlag<>0 And Shadow\enabled<>0 And Shadow\castersCount>0 Then
		...
		If Shadow\receiversCount>0 Then
			...
			For r = Each ShadowReceiver
				r\attached = 0
				If r\enabled<>0 Then
					If EntityInView( r\entity, ShadowCameraDefault ) Then
		...
		For c = Each ShadowCaster
			c\attached = 0
			If c\enabled<>0 Then
				If EntityInView( c\entity, ShadowCameraDefault ) Then
...
Function CreateShadowCaster% (caster_entity%, showAuto%=0)
...
Function AttachShadowReceiver% (receiver_entity%, showAuto%=0)
