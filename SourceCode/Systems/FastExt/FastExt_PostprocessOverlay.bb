...
; Загрузим текстуру для эффекта
; Load texture for postprocess effect 'Overlay'
AlphaTexture = LoadTexture ("media\Alien.png",1+2)


; Переменные для настройки эффекта
; Variables for adjusting the effect
	Global OverlayAlpha# = 0.5
	Global OverlayMethod = 0
	Global OverlayAlphaTexture = AlphaTexture
	Global OverlayRedColor = 64
	Global OverlayGreenColor = 255
	Global OverlayBlueColor = 64
...
While Not KeyDown(1)
	...
	; Read input keys and change effect variables
		...
		If KeyHit(2)>0 And OverlayAlpha<1 Then OverlayAlpha = OverlayAlpha + 0.1
		If KeyHit(3)>0 And OverlayAlpha>0 Then OverlayAlpha = OverlayAlpha - 0.1
		If KeyHit(4)>0 And OverlayMethod<2 Then OverlayMethod = OverlayMethod + 1
		If KeyHit(5)>0 And OverlayMethod>0 Then OverlayMethod = OverlayMethod - 1		
		If KeyHit(6) Then OverlayAlphaTexture = AlphaTexture - OverlayAlphaTexture
	
	
	
	; Customize effect
	CustomPostprocessOverlay OverlayAlpha, OverlayMethod, OverlayRedColor, OverlayGreenColor, OverlayBlueColor, OverlayAlphaTexture
	
	
	RenderPostprocess FE_Overlay		; <<<< 	Рендерим пост-процесс эффекты после команды RenderWorld
...
Function DrawHelper()
	...
	Else
		...
		Text 10,70,"keys 1/2 - increase/decrease Overlay alpha value: "+Str(OverlayAlpha)
		Text 10,90,"key 3/4 - increase/decrease Overlay method: "+Str(OverlayMethod)
		Text 10,110,"key 5 - enable/disable Overlay alpha-texture: $"+Hex(OverlayAlphaTexture)
	EndIf
End Function