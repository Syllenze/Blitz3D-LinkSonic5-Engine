...
Global RN_TimeoutPeriod = 20000, RN_TimeoutCheck = MilliSecs()
...
Type RN_Host
  Field Stream
  Field ToConfirm$[255]    ; Buffer for reliable messages needing to be confirmed
  Field ConfirmTime[255]   ; How long since the reliable messages were sent
  Field Confirmed[255]     ; Buffer for reliable message IDs received and confirmed (so they can be ignored if resent)
  Field ConfirmedTime[255] ; How long since reliable messages were received and confirmed
  Field ConfirmedIP[255], ConfirmedPort[255] ; To make sure that clients using the same confirm ID do not get mixed up
...
Type RN_Player
  ...
  Field Confirmed[255]     ; Buffer for reliable message IDs received and confirmed (so they can be ignored if resent)
  Field ConfirmedTime[255] ; How long since reliable messages were received and confirmed
  Field ConfirmedID[255]   ; To make sure that people using the same confirm ID do not get mixed up
  Field LastConfirm, LastConfirmed ; The last confirmed/to-confirm slot used
...
Function RN_Connect(Host$, HostPort, MyPort, MyName$, MyData$, LogFile$ = "", Append = True)
  ...
  P\LastConfirm = Rand(1, 255) : P\LastConfirmed = 1
...
Function RN_StartHost(LocalPort, GameData$, MaximumPlayers, LogFile$ = "", Append = True)
  ...
  H\LastConfirm = Rand(1, 255) : H\LastConfirmed = 1
  ; Open log
...
Function RN_Update()
  ...
  For H.RN_Host = Each RN_Host
    ...
    While IP <> 0
      ...
      TID$ = Chr$(ReadByte(H\Stream))
      TID$ = TID$ + Chr$(ReadByte(H\Stream))
      ToID = RN_IntFromStr(TID$)
      ...
      Next
      ; Traffic counter
      H\BytesIn = H\BytesIn + Bytes + 8
      ; Get client message is from
      FromID = -1
      For C.RN_Client = Each RN_Client
        If C\Connection = Handle(H) And C\IP = IP And C\Port = UDPMsgPort(H\Stream)
          FromID = C\ID : C\LastMsgReceived = MilliSecs() : Exit
        EndIf
      Next
      ; Confirm if message is reliable
      If Confirm <> 0
        RN_Send(Handle(H), RN_UDPReply, RN_Confirmation, RN_StrFromInt$(Confirm, 1), False, 0, %10000000)
        ; Check whether I have already received this message (i.e. confirmation packet was lost)
        For i = 0 To 255
          If H\ConfirmedIP[255] = IP And H\ConfirmedPort[255] = UDPMsgPort(H\Stream)
        ...
        If AlreadyConfirmed = False
          ...
          If H\LastConfirmed > 255 Then H\LastConfirmed = 0
        EndIf
      EndIf
        ...
        If (MsgType And %10000000) = 0
          ...
          If ToID = RN_Host
            ...
            If H\LogFile <> 0 And MsgType <> 130
              WriteLine H\LogFile, "[RECEIVED] From: #" + FromID + " Type: " + MsgType + "  [" + Str$(Len(MessageData$)) + "]"
            EndIf
          ; To all
          ElseIf ToID = RN_All
            ; Local message
          ...
          Select MsgType And %01111111
            ...
            Case RN_Confirmation
              If Len(MessageData$) = 1
                ...
                If ID <= 255 And ID >= 0 Then H\ToConfirm$[ID] = ""
              EndIf
          End Select
        EndIf
      EndIf
    ...
    For i = 1 To 255
      ...
      If MilliSecs() - H\ConfirmTime[i] > 400 And RN_StringsEqual(H\ToConfirm$[i], "") = False
  ...
  For P.RN_Player = Each RN_Player
    ...
    While IP <> 0
      ...
      FID$ = Chr$(ReadByte(P\Stream))
      FID$ = FID$ + Chr$(ReadByte(P\Stream))
      FromID = RN_IntFromStr(FID$)
      ...
      Next
      ; Traffic counter
      P\BytesIn = P\BytesIn + Bytes + 8
      ; Send confirmation if it's reliable
      If Confirm <> 0
        RN_Send(Handle(P), RN_Host, RN_Confirmation, RN_StrFromInt$(Confirm, 1), False, 0, %10000000)
        ; Check whether I have already received this message (i.e. confirmation packet was lost)
        For i = 0 To 255
        ...
        If AlreadyConfirmed = False
          ...
          If P\LastConfirmed > 255 Then P\LastConfirmed = 0
        Else
      ...
      If AlreadyConfirmed = False
        ...
        If P\LogFile <> 0 And MsgType <> 130
          WriteLine P\LogFile, "[RECEIVED] From: #" + FromID + " Type: " + MsgType + "  [" + Str$(Len(MessageData$)) + "]"
        EndIf
        ...
        Else
          Select MsgType And %01111111
            ...
            Case RN_Confirmation
              If Len(MessageData$) = 1
                ID = RN_IntFromStr(MessageData$)
                If ID <= 255 And ID >= 0 Then P\ToConfirm$[ID] = ""
              EndIf
          End Select
        EndIf
      EndIf
    ...
    For i = 1 To 255
      ; Resend unconfirmed reliable messages after a certain time
      If MilliSecs() - P\ConfirmTime[i] > 400 And RN_StringsEqual(P\ToConfirm$[i], "") = False
...
Function RN_Send(Connection, Destination, MessageType, MessageData$, ReliableFlag = 0, PlayerFrom = 0, DoNotUse = 0, ConfirmID = -1)
  ...
  If H <> Null
    ...
    If ReliableFlag = True
      If ConfirmID > -1
        Packet$ = Packet$ + Chr$(ConfirmID)
        H\ConfirmTime[ConfirmID] = MilliSecs()
      Else
        Packet$ = Packet$ + Chr$(H\LastConfirm)
      EndIf
    Else
      Packet$ = Packet$ + Chr$(0)
    EndIf
    ...
    Else
      ...
      If DestIP = 0 Then RuntimeError "RN_UDPReply message sent when no message received!"
    EndIf
	; Record message details in case it has to be re-sent
    If ReliableFlag = True And ConfirmID = -1
      ...
      H\ConfirmTime[H\LastConfirm] = MilliSecs()
      H\LastConfirm = H\LastConfirm + 1
      If H\LastConfirm > 255 Then H\LastConfirm = 1
    EndIf
    ; Traffic counter
  ...
  ElseIf P <> Null
    ...
    If ReliableFlag = True
      If ConfirmID > -1
        Packet$ = Packet$ + Chr$(ConfirmID)
        P\ConfirmTime[ConfirmID] = MilliSecs()
      Else
        Packet$ = Packet$ + Chr$(P\LastConfirm)
        P\ToConfirm$[P\LastConfirm] = RN_StrFromInt$(MessageType Or DoNotUse, 1) + RN_StrFromInt$(Destination, 2)
        P\ToConfirm$[P\LastConfirm] = P\ToConfirm$[P\LastConfirm] + MessageData$
        P\ConfirmTime[P\LastConfirm] = MilliSecs()
        P\LastConfirm = P\LastConfirm + 1
        If P\LastConfirm > 255 Then P\LastConfirm = 1
      EndIf
    Else
      Packet$ = Packet$ + Chr$(0)
    EndIf
