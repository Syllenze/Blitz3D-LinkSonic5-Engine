...
; /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
	...
	; Player_Motion
	; =========================================================================================================
	Function Player_Motion2(p.tPlayer2, d.tDeltaTime)
		...
		Ground = Player_TestCollisions2(p, d)
		...
		If (p\Objects\Mesh2 <> 0) Then PositionEntity(p\Objects\Mesh2, EntityX(p\Objects\Entity2), EntityY(p\Objects\Entity2), EntityZ(p\Objects\Entity2))
	;	If (p\Objects\Mesh2a <> 0) Then PositionEntity(p\Objects\Mesh2a, EntityX(p\Objects\Entity2)-5, EntityY(p\Objects\Entity2), EntityZ(p\Objects\Entity2)+3)
	;	If (p\Objects\Mesh2b <> 0) Then PositionEntity(p\Objects\Mesh2b, EntityX(p\Objects\Entity2)+5, EntityY(p\Objects\Entity2), EntityZ(p\Objects\Entity2)-3)
		
		
		If (p\Objects\Mesh2 <> 0) Then RotateEntity(p\Objects\Mesh2, 0, p\Animation\Direction#, 0)
	;	If (p\Objects\Mesh2a <> 0) Then RotateEntity(p\Objects\Mesh2a, 0, p\Animation\Direction#, 0)
	;	If (p\Objects\Mesh2b <> 0) Then RotateEntity(p\Objects\Mesh2b, 0, p\Animation\Direction#, 0)
		
		
		If (p\Objects\Mesh2 <> 0) Then AlignToVector(p\Objects\Mesh2, p\Animation\Align\x#, p\Animation\Align\y#, p\Animation\Align\z#, 2)
	;	If (p\Objects\Mesh2a <> 0) Then AlignToVector(p\Objects\Mesh2a, p\Animation\Align\x#, p\Animation\Align\y#, p\Animation\Align\z#, 2)
	;	If (p\Objects\Mesh2b <> 0) Then AlignToVector(p\Objects\Mesh2b, p\Animation\Align\x#, p\Animation\Align\y#, p\Animation\Align\z#, 2)

		
		; Now, just move over the character to the new position, based on it's speed.
		If (p\Motion\Ground=True) Then
			MoveEntity(p\Objects\Entity2, p\Motion\Speed\x#*d\Delta, p\Motion\Speed\y#*d\Delta-(0.015+(Vector_Length#(p\Motion\Speed)*0.33*d\Delta)), p\Motion\Speed\z#*d\Delta)
		Else	
			MoveEntity(p\Objects\Entity2, p\Motion\Speed\x#*d\Delta, p\Motion\Speed\y#*d\Delta, p\Motion\Speed\z*d\Delta)
		EndIf
	End Function
	...
	; =========================================================================================================
	Function Player_TestCollisions2(p.tPlayer2, d.tDeltaTime)
		...
		For i = 1 To CountCollisions(p\Objects\Entity2)
			; Clear current collision normal
			Vector_Set(CollisionNormal, 0, 0, 0)
			ShouldAlign = False
			
			; Add vertex normals to our normal vector
			Select GetEntityType(CollisionEntity(p\Objects\Entity2, i))
				Case COLLISION_WORLD_POLYGON_ALIGN
					; Setup
					ShouldAlign = True
					
					; Acquire collided surface & triangle
					Surface 	= CollisionSurface(p\Objects\Entity2, i)
					Triangle 	= CollisionTriangle(p\Objects\Entity2, i)
				...
				Case COLLISION_WORLD_BOX
					...
					CollisionNormal\x# = CollisionNX#(p\Objects\Entity2, i)
					CollisionNormal\y# = CollisionNY#(p\Objects\Entity2, i)
					CollisionNormal\z# = CollisionNZ#(p\Objects\Entity2, i)
	...
	; Player_Align
	; =========================================================================================================
	Function Player_Align2(p.tPlayer2)
		RotateEntity(p\Objects\Entity2, 0, 0, 0)
		AlignToVector(p\Objects\Entity2, p\Motion\Align\x#, p\Motion\Align\y#, p\Motion\Align\z#, 2)
	End Function
	...
	; =========================================================================================================
	Function Player_ConvertAirToGround2(p.tPlayer2)
		TFormVector(p\Motion\Speed\x#, p\Motion\Speed\y#, p\Motion\Speed\z#, Game\Stage\Gravity, p\Objects\Entity2)
	...
	; =========================================================================================================
	Function Player_ConvertGroundToAir2(p.tPlayer2)
		TFormVector(p\Motion\Speed\x#, p\Motion\Speed\y#, p\Motion\Speed\z#, p\Objects\Entity2, Game\Stage\Gravity)
		p\Motion\Speed\x# = TFormedX#()
