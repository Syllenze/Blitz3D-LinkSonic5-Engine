...
; /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
	...
	Const COLLISION_WORLD_DIRCORRECT	=	17
	
	Const COLLISION_WORLD_FALL			=	18
...
Function LevelSelect()
	...
	;	CloseFile(fileout)
		...
		editipjoin				= GUI_CreateEdit(grp1, 95, 195, 30, Read17)
		editipjoin1				= GUI_CreateEdit(grp1, 125, 195, 30, Read19)
		editipjoin2				= GUI_CreateEdit(grp1, 155, 195, 30, Read20)
		editipjoin3				= GUI_CreateEdit(grp1, 185, 195, 30, Read21)
...
;		If (GameStarted = 0) Then
		...
		EndIf
;	Else
;		GUI_MsgBox("Error", "You have previously started a game." + Chr(10) + "Please re-start the game to use online features.")
;	EndIf
...
;	If (GameStarted = 0) Then
		...
		If GUI_Message(editipjoin, "GetText") = 0 Then
			BP_JoinSession (GUI_Message(editname, "GetText"),80,"",2222)
			IP$ = ""
		Else
			BP_JoinSession (GUI_Message(editname, "GetText"),80,GUI_Message(editipjoin, "GetText")+"."+GUI_Message(editipjoin1, "GetText")+"."+GUI_Message(editipjoin2, "GetText")+"."+GUI_Message(editipjoin3, "GetText"),2222)
			IP$ = GUI_Message(editipjoin, "GetText")+"."+GUI_Message(editipjoin1, "GetText")+"."+GUI_Message(editipjoin2, "GetText")+"."+GUI_Message(editipjoin3, "GetText")	
		EndIf
	;		IP$ = ""
	;	Else
	;		IP$ = GUI_Message(editipjoin, "GetText")+"."+GUI_Message(editipjoin1, "GetText")+"."+GUI_Message(editipjoin2, "GetText")+"."+GUI_Message(editipjoin3, "GetText")
	...
	;	BP_JoinSession (GUI_Message(editname, "GetText"),My_Port,IP$,2222)
...
FootStepEnabled = GUI_Message(footstepbox, "GetChecked")
	...
	WriteLine(fileout, GUI_Message(editname, "GetText")) : PlayerName$ = GUI_Message(editname, "GetText")
	WriteLine(fileout, GUI_Message(editipjoin1, "GetText"))
	WriteLine(fileout, GUI_Message(editipjoin2, "GetText"))
	WriteLine(fileout, GUI_Message(editipjoin3, "GetText"))
...
Else
	...
	;	If UseStageChar > 0 Then Load_Characters_List(ModelSelect$)
...
End Function
	...
	;	CameraProjMode CubeCamera, 0 ;Setting ProjectionMode to 0 deactives the camera
		 ...
		 ; ----
	;	CubeMapTex = CreateTexture (512, 512, 1+16+32+256 + FE_RENDER + FE_ZRENDER) 
...
;Function Game_Stage_Setup()
				...
				Case "information"
					...
					For j = 1 To xmlNodeChildCount(RootChildNode)
						...
						;		If (ScenePosition<>0) Then PositionEntity(Game\Stage\Properties\Sun, Float(xmlNodeAttributeValueGet(ScenePosition, "x")), Float(xmlNodeAttributeValueGet(ScenePosition, "y")), Float(xmlNodeAttributeValueGet(ScenePosition, "z")))
							...
							;		BumpMapTex  = LoadTexture("Textures/Water_Bump.png")
							;		TextureBlend  BumpMapTex, FE_BUMPLUM
									
								;	EntityTexture CMWater, CubeMapTex;, 0, 1
							;		EntityAlpha CMWater,.55
									
							;		EntityTexture CMWater, BumpMapTex, 0, 0
							;		EntityTexture CMWater, CubeMapTex, 0, 1
					...
					For j = 1 To xmlNodeChildCount(RootChildNode)
						...
						Select xmlNodeNameGet$(SceneChildNode)
							Case "mesh"								
								...
								MeshFall		= xmlNodeAttributeValueGet(SceneChildNode, "fall")
								...
								DirCorrect		= xmlNodeAttributeValueGet(SceneChildNode, "dircorrect")
							...
							EndIf
								...
								If (DirCorrect = True) Then
									EntityType(Mesh, COLLISION_WORLD_DIRCORRECT)
									EntityPickMode(Mesh, 2)
								End If
								...
								If (MeshFall = True) Then
									EntityType(Mesh, COLLISION_WORLD_FALL)
									EntityPickMode(Mesh, 2)
								End If
    ...
    ; Setup position, rotation and scale.
							...
							Case "light"
								...
								Game\LightNumber = Game\LightNumber + 1
							...
							;	For l.tGame_Lights = Each tGame_Lights
								...
								Select LightType$
										...
										Case "directional"
											If (Game\Lights[Game\LightNumber] = Null) Then
												Game\Lights[Game\LightNumber] = New tGame_Lights
												Game\Lights[Game\LightNumber]\Light = CreateLight(1)
											End If
											
									Case "point"
										If (Game\Lights[Game\LightNumber] = Null) Then
											Game\Lights[Game\LightNumber] = New tGame_Lights
											Game\Lights[Game\LightNumber]\Light = CreateLight(2)
											End If
											
									Case "spot"
										If (Game\Lights[Game\LightNumber] = Null) Then
											Game\Lights[Game\LightNumber] = New tGame_Lights
											Game\Lights[Game\LightNumber]\Light = CreateLight(3)
											End If
											
								End Select
							;	LightRange(Game\Stage\Properties\Light, Float(xmlNodeAttributeValueGet(SceneChildNode, "range")))
								LightRange(Game\Lights[Game\LightNumber]\Light, Float(xmlNodeAttributeValueGet(SceneChildNode, "range")))
								
								; Setup position, rotation and scale.
								LightPosition = xmlNodeFind("position", SceneChildNode)
								If (LightPosition<>0) Then PositionEntity(Game\Lights[Game\LightNumber]\Light, Float(xmlNodeAttributeValueGet(LightPosition, "x")), Float(xmlNodeAttributeValueGet(LightPosition, "y")), Float(xmlNodeAttributeValueGet(LightPosition, "z")))
								LighRotation = xmlNodeFind("rotation", SceneChildNode)
								If (LighRotation<>0) Then RotateEntity(Game\Lights[Game\LightNumber]\Light, xmlNodeAttributeValueGet(LighRotation, "pitch"), xmlNodeAttributeValueGet(LighRotation, "yaw"), xmlNodeAttributeValueGet(LighRotation, "roll"))
								LightCol = xmlNodeFind("color", SceneChildNode)
								If (LightCol<>0) Then LightColor(Game\Lights[Game\LightNumber]\Light, xmlNodeAttributeValueGet(LightCol, "r"), xmlNodeAttributeValueGet(LightCol, "g"), xmlNodeAttributeValueGet(LightCol, "b"))
					...
					;	Next
						...
						Case "sound"
							...
							Case "object"
								...
								Select xmlNodeAttributeValueGet(SceneChildNode, "type")
									...
									EndIf
										...
										Game\Stage\startaction$ = (xmlNodeAttributeValueGet(SceneChildNode, "action"))
									...
									Case "ring"
										; Update progress bar
										Game_Stage_UpdateProgressBar("Loading object n."+j+": Yellow Spinny Thingy", Float#(j)/Float#(xmlNodeChildCount(RootChildNode)))
									...
									Case "bumper"
										; Update progress bar
										Game_Stage_UpdateProgressBar("Loading object n."+j+": Bumper... Seriously who ever uses these?", Float#(j)/Float#(xmlNodeChildCount(RootChildNode)))
							...
							Case "enemy"
								...
								SceneEnemy = xmlNodeFind("enemy", SceneChildNode)
								If (SceneEnemy<>0) Then
									model$ = xmlNodeAttributeValueGet(SceneEnemy, "model")
									attacks = xmlNodeAttributeValueGet(SceneEnemy, "attacks")
								End If
								
								obj.tObject = Object_Enemy_Create(positionX#, positionY#, positionZ#, rotationX#, rotationY#, rotationZ#, attacks, model$)
							...
							Case "npc"
										; Update progress bar
								Game_Stage_UpdateProgressBar("Loading object n."+j+": Non-Playable Character", Float#(j)/Float#(xmlNodeChildCount(RootChildNode)))
							...
							Case "dashpadnew"
										; Update progress bar
								Game_Stage_UpdateProgressBar("Loading object n."+j+": Optimised Dash Pad", Float#(j)/Float#(xmlNodeChildCount(RootChildNode)))
							...
							Case "dashramp"
										; Update progress bar
								Game_Stage_UpdateProgressBar("Loading object n."+j+": Boost Ramp", Float#(j)/Float#(xmlNodeChildCount(RootChildNode)))
							...
							Case "pickup"
								...
								; Setup model.
								SceneProp = xmlNodeFind("prop", SceneChildNode)
								If (SceneProp<>0) Then
								model$ = xmlNodeAttributeValueGet(SceneProp, "model")
								animated = xmlNodeAttributeValueGet(SceneProp, "animated")
							End If
								
							obj.tObject = Object_PickUp_Create(positionX#, positionY#, positionZ#, scaleX#, scaleY#, scaleZ#, model$, animated, sound$)
								
							Case "destspring"
										; Update progress bar
								Game_Stage_UpdateProgressBar("Loading object n."+j+": Auto-Spring", Float#(j)/Float#(xmlNodeChildCount(RootChildNode)))
							...
							Case "musictrigger"
								...
								If (SceneMusic<>0) Then
									...
									music$ = xmlNodeAttributeValueGet(SceneMusic, "music")
									loops = xmlNodeAttributeValueGet(SceneMusic, "loops")
									
								End If
								
								obj.tObject = Object_Music_Create(positionX#, positionY#, positionZ#, music$, radius#, loops)
				...
				;			Case "player2"
				;						; Update progress bar
				;				Game_Stage_UpdateProgressBar("Loading object n."+j+": Ready for Player 2", Float#(j)/Float#(xmlNodeChildCount(RootChildNode)))
				;				
				;				positionX# = 0
				;				positionY# = 0
				;				positionZ# = 0
				...
				;				End If
				;				
				;				obj.tObject = Object_Player2_Create(positionX#, positionY#, positionZ#, rotationX#, rotationY#, rotationZ#, 1)
	...
	;	PlaySound(Game\Stage\Properties\Music)
		...
		Collisions(COLLISION_PLAYER, COLLISION_WORLD_FALL, 			2, 2)
		Collisions(COLLISION_PLAYER, COLLISION_WORLD_DIRCORRECT, 	2, 2)
		
		Collisions(COLLISION_PLAYER, COLLISION_WORLD_RAIL, 			2, 2)
		
		Collisions(COLLISION_GRINDAFFECTOR, COLLISION_WORLD_RAIL, 			2, 2)
		Collisions(COLLISION_GRINDAFFECTOR, COLLISION_WORLD_DIRCORRECT, 	2, 2)
...
;		EndIf
	...
	;	If (Game\Stage\Properties\Snow = 1) Then SetEmitter(p\Objects\TempPiv, Game\Stage\Snow)
		...
		;	For m.MeshStructure = Each MeshStructure : For c.tCamera = Each tCamera : RenderEntity (m\Entity, c\Entity) : Next : Next
			...
			For p.tPlayer = Each tPlayer
				If Online = 1 Then
					obj.tObject = Object_Player2_Create(EntityX(p\Objects\Entity),EntityY(p\Objects\Entity),EntityZ(p\Objects\Entity),0,0,0, 1)
				EndIf
			Next
			
			; PLAY JOIN SOUND
			If Online = 1 Then BP_UDPMessage(0,14,JoinSound)
		
		;PostEffect_Create_MotionBlur(0.84)
End Function

;Global Player2PositionX
;Global Player2PositionY
;Global Player2PositionZ
;Global Player2RotationX
;Global Player2RotationY
;Global Player2RotationZ
;Global Player2Animation
...
Function Game_Stage_Step()
	...
	If online=1
		...
		If XX > 25		
			...
			BP_UDPMessage(0,8,PlayerConnected)
			BP_UDPMessage(0,9,Gameplay_Character)
			BP_UDPMessage(0,10,PlayerName$)
		;	BP_UDPMessage(0,11,PlayerChatText$)
			BP_UDPMessage(0,12,Chatting)
			BP_UDPMessage(0,13,SentMessage)
			HandleMessages()
			XX = 0
		EndIf 
	EndIf
	
;	PauseTimer = PauseTimer + GlobalTimer
;	PauseTimer1 = PauseTimer1 + GlobalTimer
	
;	If Paused = True And KeyHit(KEY_F12) Then
	;	PauseTimer = 0
	;	PauseTimer1 = 0
;		Paused = False
	;	PauseTimer = PauseTimer + GlobalTimer
...
;	If PauseTimer > PauseTimer1 Then Pause = True
		
		; SEND PLAYER BACK TO START
	
	If (Paused = False Or Online = 1) Then
		...
		For p.tPlayer = Each tPlayer
			If (KeyHit(KEY_HOME) And p\UsingTerminal = 0) Then
				
				HomeButton = 1
				
				Game\Stage\StartX# = EntityX#(p\Objects\Entity)
				Game\Stage\StartY# = EntityY#(p\Objects\Entity)
				Game\Stage\StartZ# = EntityZ#(p\Objects\Entity)
				
				Game\Stage\DefStartX# = EntityX#(p\Objects\Entity)
				Game\Stage\DefStartY# = EntityY#(p\Objects\Entity)
				Game\Stage\DefStartZ# = EntityZ#(p\Objects\Entity)
				
				p\CheckpointX# = EntityX#(p\Objects\Entity)
				p\CheckpointY# = EntityY#(p\Objects\Entity)
				p\CheckpointZ# = EntityZ#(p\Objects\Entity)
				
				Game\StageSelecting = 0 : Game\StartGame = 1 : p\UsingTerminal = 0 : StartGame(Game\Stage\GoalDestination, Sound = 1)
	...
	EndIf
		...
		For p.tPlayer = Each tPlayer : p\Frame = RecursiveAnimTime (p\Objects\Mesh) : Next
			...
			StopChannel(Game\MusicChn)
			If (Game\Stage\Properties\MusicLoops = 1) Then Game\MusicChn = PlaySound(Game\Stage\Properties\Music)
		EndIf
			;------------------
	...
	For p.tPlayer = Each tPlayer
		...
		If ((p\LoseLife = 1) And (Not p\Action = ACTION_HURT)) Then
			...
			;	p\Action = ACTION_FALL
				...
				If Game\Stage\Hub = 0 Then Game\Gameplay\Lives = Game\Gameplay\Lives - 1
...
Next
	...
	;	Next
		...
		; Render world
		If (KeyHit(KEY_F5)) Then WherePlayers = 1-WherePlayers
	...
	;			For c.tCamera = Each tCamera
			...
			If BetaTapeMode = 1 Then CustomPostprocessContrast(.64,1,205,205,225,2) : Else CustomPostprocessContrast(.44,0,225,225,225,2)
		...
		;	CustomPostprocessBlurZoom 0.5, 0.5, 125, p\Motion\GroundSpeed#/10, 4, 1,  255,  255,  255, FadeTexture
			...
			;	RenderPostprocess FE_Posterize
...
;	If ShowHUD = 1 Then
	...
	If Online = 1 Then
		For o.tObject = Each tObject
			For p.tPlayer = Each tPlayer
;		For c.tCamera = Each tCamera
				If p\Objects\Mesh <> 0 Then LabelEntity (c\Entity, p\Objects\Mesh, PlayerName$, 7)
				If o\Entity1 <> 0 Then LabelEntity (c\Entity, o\Entity1, Player2Name$, 7)
				
			;	If p\Objects\Mesh <> 0 Then LabelEntity (c\Entity, p\Objects\Mesh, PlayerChatText$, 4.5)
			;	If o\Entity1 <> 0 And EntityDistance(p\Objects\Entity, o\Entity) < 140 Then LabelEntity (c\Entity, o\Entity1, Player2ChatText$, 4.5)
;		Next
		Next
	Next
EndIf
...
;	EndIf
	...
	;	WrapText(250*GAME_WINDOW_SCALE#, 425*GAME_WINDOW_SCALE#, "BETA", 110*GAME_WINDOW_SCALE#,0,0,0, 255,0,0, 1)
		...
		For p.tPlayer = Each tPlayer
			...
			If (p\UsingTerminal = 1 Or Chatting = 1) Then
		...
		;		WrapText(100*GAME_WINDOW_SCALE#, 190*GAME_WINDOW_SCALE#, "Score: " + Game\Gameplay\Score, 330*GAME_WINDOW_SCALE#,0,0,0, 90,200,255, 1)
			...
			If Online = 1 Then
				...
				ChatTimer = ChatTimer + GlobalTimer
				If ChatTimer > 9000 Then PlayerChatText$=""
				
				; ONLINE CHAT
				If (KeyHit(KEY_T) And Chatting = 0 And Paused = 0) Then FlushKeys : PlayerChatText$="" : Chatting = 1
				...
				If Chatting = 1 Then
					...
					AboutToChat()
				;	PlayerChatText$ = cc_text_input$("",100*GAME_WINDOW_SCALE#, 288*GAME_WINDOW_SCALE#,10,10)
				;	cc_text_input$("",100*GAME_WINDOW_SCALE#, 288*GAME_WINDOW_SCALE#,10,10)
		...
		EndIf
	;			WrapText(200*GAME_WINDOW_SCALE#, 30*GAME_WINDOW_SCALE#, "IP Address: " + BP_GetHostIP(), 330*GAME_WINDOW_SCALE#,0,0,0, 200,200,255, 1)
	;	WrapText(200*GAME_WINDOW_SCALE#, 30*GAME_WINDOW_SCALE#, EntityPitch(p\Objects\Mesh), 330*GAME_WINDOW_SCALE#,0,0,0, 200,200,255, 1)
		...
		If (p\BB_CanDo = 1) Then WrapText(20*GAME_WINDOW_SCALE#, 165*GAME_WINDOW_SCALE#, "ActionB: Barrier Blast", 130*GAME_WINDOW_SCALE#,0,0,0, 255,0,0, 1)
			...
			;	WrapText(5*GAME_WINDOW_SCALE#, 45*GAME_WINDOW_SCALE#, "'.' Copy Rotation (Float Value)", 200*GAME_WINDOW_SCALE#,255,255,255, 255,0,0, 1)
				...
				WrapText(230*GAME_WINDOW_SCALE#, 105*GAME_WINDOW_SCALE#, "Top Speed: " + COMMON_XZTOPSPEED# + "Acceleration: " + COMMON_XZACCELERATION#, 330*GAME_WINDOW_SCALE#,0,0,0, 200,200,255, 1)
...
End Function
	...
	;	CharType = 0
		...
		;	If (Game\CreatedCharacter = 1) Then
			...
			FreeTemplate(p\Particles\Land)
		;	FreeTemplate(p\Particles\SDash)
			FreeTemplate(p\Particles\Step1)
			FreeTemplate(p\Particles\Step2)
			FreeTemplate(p\Particles\Rock)
			FreeParticles()
...
Next
	...
	;	FreeParticles()
			...
			;	If (o\ObjType=1) Then FreeEntity(o\IValues[0]) : FreeEntity(o\IValues[1]) : FreeEntity(o\IValues[2]) : FreeEntity(o\IValues[3])
				...
				If o\Entity1 <> 0 Then FreeEntity o\Entity1
				If o\Where <> 0 Then FreeEntity o\Where
				If o\Homer <> 0 Then FreeEntity o\Homer
	...
	;Delete Lights
		...
		;	BASS_Free()
			...
			;	If Sounds[i]\Sound <> 0 Then Sounds[i]\Sound = 0
				If Sounds1[i]\Sound <> 0 Then FreeSound Sounds1[i]\Sound
...
;		FreeEntity Game\Stage\Root
			...
			Select Game\PickStage
				...
				Case 0 : Load_Characters_List(ModelSelect$) : Game\State = GAME_STATE_START : SetFont SystemFont
			End Select
...
;If prevx#<>0.0 Then
	...
	TiltTimer# = TiltTimer# + GlobalTimer
	
	If TiltTimer > TiltDuration# Then StageIsTilted=True
...
;	PlaySound Sound_Siren
	...
	If TiltTimer > TiltDuration# And TiltDuration <> -1 Then StageIsTilted=False : TiltX# = 0 : TiltY# = 0 : TiltZ# = 0
;End If
End Function
...
Function HandleMessages ()
	...
	;	For i = 1 To 10
		Select msg\msgType
			...
			Case 8		;Player 2 Arrival
				Player2Connected = (msg\msgData)
			Case 9		;Player 2 Character
				Player2Character = (msg\msgData)
			Case 10		;Player 2 Name
				Player2Name$ = (msg\msgData)
			Case 11		;Player 2 Chat
				Player2ChatText$ = (msg\msgData)
			;	If Player2SentMessage = 1 Then Info (Player2Name$ + ": " + msg\msgData)
				Info (Player2Name$ + ": " + msg\msgData) : PlaySound(Sound_Message)
		;	Case 11  ;a chat packet
		;		p.pdata = Find_pdata(msg\msgFrom)
		;		Info (p\Name + ":" + msg\msgData)			Case 12  ;Player 2 Chatting
				Player2Chatting = (msg\msgData)
			Case 13  ;Player 2 Sending Message
				Player2SentMessage = (msg\msgData)
			Case 14  ;Player 2 Joined
				JoinSound = (msg\msgData)
...
Function LabelEntity (camera, entity, label$, Offset#)
	
	;Timer1 = Timer1 + GlobalTimer
	
	If EntityInView (entity, camera)
		CameraProject camera, EntityX (entity), EntityY (entity)+Offset#, EntityZ (entity)
		w = StringWidth (label$)
	...
	;	Rect x, y-40, w + 2, h + 2, 1
		Color 255, 255, 255
	;	If Timer1 > 30 Then Text x, y-40, label$ : Timer1 = 0
		Text x, y-40, label$
	EndIf
End Function
